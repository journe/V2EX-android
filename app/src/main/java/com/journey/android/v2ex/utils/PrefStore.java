...
public class PrefStore implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    static {
        instance = new PrefStore(V2exApplication.getInstance());
    }
    ...
    private void initPref() {
        ...
        if (version == 2) {
            final File path = V2exApplication.getInstance().getFileStreamPath("CookiePrefsFile.xml");
            if (path.isFile()) {
                path.delete();
            }
    ...
    public static void requestBackup() {
        final BackupManager manager = new BackupManager(V2exApplication.getInstance());
        manager.dataChanged();
    }
