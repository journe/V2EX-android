...
object LoginParser {
	...
	fun parseSignInData(doc: Document): SignInFormData {
		...
		signInFormData.next = content.select("input[name=next]")
			.attr("value")
		signInFormData.once = content.select("input[name=once]")
			.attr("value")
			.toInt()
		signInFormData.account = content.select("input[type=text]")[0]
			.attr("name")
		signInFormData.captcha = content.select("input[type=text]")[1]
			.attr("name")
		signInFormData.password = content.select("input[type=password]")
			.attr("name")
	...
	@JvmStatic
	fun parseLoginError(doc: Document): LoginResult {
		val content = doc.body()
			.selectFirst("#Wrapper")
			.selectFirst(".content")
			.selectFirst(".box")
		return LoginResult().apply {
			problem = content.selectFirst(".problem").text()
		}
	}
}