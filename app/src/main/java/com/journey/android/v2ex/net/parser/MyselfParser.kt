...
import com.journey.android.v2ex.model.jsoup.LoginResult
...
object MyselfParser {
	...
	fun parseLoginResult(doc: Document): LoginResult {
		val tr = JsoupObjects(doc).body().child("#Wrapper").child(".content").child("#Rightbar")
			.dfs("tr").first()
	...
	fun parseDoc(doc: Document, pageType: PageType): MySelfInfo? {
		...
		val box = JsoupObjects(doc).body().child("#Wrapper").child(".content").child("#Rightbar")
			.child(".box").first()
	...
	private fun parseTopic(doc: Document): MySelfInfo? {
		val ele =
			JsoupObjects(doc).body().child("header", "#site-header-menu").bfs("a[href=/settings]")
				.firstOrNull()
	...
	private fun hasAwardInTab(box: Element): Boolean {
		val ele =
			JsoupObjects(box.parent()).child(".box").child(".inner").child(".fa-gift").firstOrNull()
		return ele != null
	}

	@JvmStatic
	fun hasAward(html: String): Boolean {
		val doc = html.toJsoup()
...
//		val elements = JsoupObjects(doc).body().child("#Wrapper").child(".content").child("#Main")
//			.child(".box").child("#my-nodes").child("a")
